...
public class AspireCouchbaseExtensionsTests : IClassFixture<CouchbaseContainerFixture>
{
    private const string DefaultConnectionName = "couchbase";

    private readonly CouchbaseContainerFixture _containerFixture;

    public AspireCouchbaseExtensionsTests(CouchbaseContainerFixture containerFixture)
    {
    ...
    [Theory]
    [InlineData("couchbase://localhost:27017")]
    public void AddCouchbaseClient_ReadsFromConnectionStringsCorrectly(string connectionString)
    {
        var builder = CreateBuilder(connectionString);

        builder.AddCouchbaseClient(DefaultConnectionName);

        using var host = builder.Build();

        var couchbaseClient = host.Services.GetRequiredService<ICouchbaseClient>();
    ...
    public void AddKeyedMongoDBDataSource_HealthCheckShouldNotBeRegisteredWhenDisabled()
    {
        ...
        builder.AddKeyedCouchbaseClient(DefaultConnectionName, settings =>
        {
    ...
    //        new KeyValuePair<string, string?>("ConnectionStrings:couchbase1", "couchbase://localhost:27011/mydatabase1"),
    //        new KeyValuePair<string, string?>("ConnectionStrings:couchbase2", "couchbase://localhost:27012/mydatabase2"),
    //        new KeyValuePair<string, string?>("ConnectionStrings:couchbase3", "couchbase://localhost:27013/mydatabase3"),
    //    ]);

    //    builder.AddCouchbaseClient("couchbase1");
    //    builder.AddKeyedCouchbaseClient("couchbase2");
    //    builder.AddKeyedCouchbaseClient("couchbase3");
